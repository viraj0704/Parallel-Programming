...
/***    Changing The Code                                                             ******/
	...
	for(int i=0;i<pairs.size();i++){
		cout << pairs[i].first << " " << pairs[i].second << " " <<  out[i] << endl;
	}
...
void* task(void* rank){
    ...
    int boxes = m/(segment * thread_count);
    
    for(int j=0; j<= boxes;j++){
        int firstrowof_box = j*segment*thread_count;
        int firstrowof_seg = firstrowof_box + (my_rank*segment);
        int lastrowof_seg = firstrowof_seg + segment;

        for(int i=firstrowof_seg;(i<lastrowof_seg && i<m);i++){
            // cout << "i " << i << " rank " << my_rank << endl;
